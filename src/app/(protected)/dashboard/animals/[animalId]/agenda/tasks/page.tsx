...
export default function AnimalAgendaTasksPage() {
  ...
  onClick={async (e) => {
    ...
    try {
      ...
      console.error("cancel-recurrence failed", err);
      return;
    }
                  ...
                  {/* <button
                    ...
                    onClick={async (e) => {
                      ...
                      const res = await fetch(
                        `${process.env.NEXT_PUBLIC_API_URL ?? ""}/api/v1/animals/${animalId}/tasks/${task.id}/cancel-recurrence`,
                        {
                          method: "PATCH",
                          headers: {
                          ...
                          body: JSON.stringify({
                            cancel_master: true,
                            comment: "Recorrência cancelada pelo utilizador",
                          }),
                        }
                      );

                      if (!res.ok) {
                        // força ver o erro no console para não ficar "silencioso"
                        const body = await res.text().catch(() => "");
                        console.error("cancel-recurrence failed", res.status, body);
                        return;
                      }
