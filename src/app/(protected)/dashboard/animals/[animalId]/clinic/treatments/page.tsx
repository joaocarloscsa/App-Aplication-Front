            ...
            >
              ...
              {t.schedules.map((s) => (
                <div
                  ...
                  className="rounded border bg-zinc-50 p-3 text-xs space-y-1"
                >
<div className="flex items-start justify-between gap-2">
  <div className="space-y-0.5">
    <p className="text-sm font-semibold text-zinc-900">
      {s.medication_name || "Medicação / Procedimento"}
    </p>

    {s.created_by && (
      <p className="text-[11px] text-zinc-500">
        Prescrito por{" "}
        <span className="font-medium">{s.created_by.name}</span>{" "}
